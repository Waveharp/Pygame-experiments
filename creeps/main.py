...
class Creep(Sprite):
	...
	def update(self, time_passed):
		""" Update the creep.
			time_passed:
				The time passed (in ms) since previous update.
		"""
	...
	def _point_is_inside(self, point):
		"""Is the point inside our creep's body?"""
	...
	def _decrease_health(self, n):
		""" Decrease my health by n (or to 0, if currently less than n)"""
...
class Game(object):
	...
	def __init__(self):
		...
		self.tile_img = pg.image.load(self.BG_TILE_IMG).convert_alpha()
		...
		self.creep_images= [
			(   pg.image.load(f1).convert_alpha(),
				pg.image.load(f2).convert_alpha())
			for (f1, f2) in self.CREEP_FILENAMES]

		explosion_img = pg.image.load('images/explosion1.png').convert_alpha()
		self.explosion_images = [
			explosion_img, pg.transform.rotate(explosion_img, 90)]
	...
	def draw_portals(self):
		entrance_sf = pg.Surface((self.entrance_rect.w, self.entrance_rect.h))
		entrance_sf.fill(Color(80, 200, 80))
		entrance_sf.set_alpha(150)
		self.screen.blit(entrance_sf, self.entrance_rect)
		
		exit_sf = pg.Surface((self.exit_rect.w, self.exit_rect.h))
	...
	def draw_walls(self):
		...
		for wall in self.walls:
			...
			pg.draw.polygon(self.screen, wallcolor,
				[   (pos_x - radius, pos_y), (pos_x, pos_y + radius),
					(pos_x + radius, pos_y), (pos_x, pos_y - radius)])
			
			if (nrow + 1, ncol) in self.walls:
				pg.draw.line(self.screen, wallcolor,
