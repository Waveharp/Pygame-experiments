...
from platakart.ui import TextSprite
...
def text_position(i):
    """ return a string with numeral and suffix:
    1st, 2nd, 3rd, etc
    """
    end = str(i)[-1]
    if end == "1":
...
class RaceTimeSprite(TextSprite):
    def __init__(self, race_manager, **kwargs):
        time = race_manager.race_clock
        super(RaceTimeSprite, self).__init__(str(time), **kwargs)
        self.race_manager = race_manager

    def update(self, dt=None):
        if not self.race_manager.race_complete:
            self.text = str(self.race_manager.race_clock/100)


class RaceViewPort(ViewPort):
    """ ViewPort with racing hud
    """
    def __init__(self, resources, kart_id, race_manager):
        super(RaceViewPort, self).__init__()
        ...
        small_font = pygame.font.Font(resources.fonts['dimitri'], 64)
        large_font = pygame.font.Font(resources.fonts['dimitri'], 96)

        # This will have to be handled after the rect is set
        rect = pygame.display.get_surface().get_rect()
        x = rect.right * .8

        msg = "{}/{}".format(laps, race_manager.number_of_laps)
        self.lap_sprite = TextSprite(msg, color, font=small_font)
        self.lap_sprite.rect.move_ip((x, 100))

        self.position_numeral_sprite = TextSprite(str(position),
                                                  color,
                                                  font=large_font)
        self.position_numeral_sprite.rect.move_ip((x, 0))

        self.position_suffix_sprite = TextSprite(text_position(position),
                                                 color,
                                                 font=small_font)
        self.position_suffix_sprite.rect.move_ip((x, 0))
        self.position_suffix_sprite.rect.move_ip(
            (self.position_numeral_sprite.rect.width, 0))

        self.hudgroup.add(self.lap_sprite)
        self.hudgroup.add(self.position_numeral_sprite)
        self.hudgroup.add(self.position_suffix_sprite)
    ...
    def setup(self):
        pub.subscribe(self.on_lap_complete, "kart.lap-complete")

    def teardown(self):
        pub.unsubscribe(self.on_lap_complete, "kart.lap-complete")

    def on_lap_complete(self, kart_id, lap):
        if kart_id == self.kart_id:
            msg = "{}/{}".format(lap, self.race_manager.number_of_laps)
            self.lap_sprite.text = msg

    def draw(self, surface, rect):
        super(RaceViewPort, self).draw(surface, rect)
        self.hudgroup.draw(surface)
        return rect

    def update(self, delta=None):
        super(RaceViewPort, self).update(delta)
        self.hudgroup.update()
...
class RaceManager(object):
    ...
    def update(self, delta):
        if self.race_complete:
            # TODO: Do some kind of fanfare sequence here before
            # switching to race result scene.

            # TODO: Create race result record to pass to race result
            # scene that contains lap and place data.
    ...
    def on_kart_entered_checkpoint(self, kart, checkpoint_shape):
        if kart.checkpoint is None:
            # The karts were like not initialized correctly
            logger.warning("Kart had no initial checkpoint!")
            return

        if self.race_complete:
            # The race is over and the karts shouldn't be lapping anymore
            logger.warning("Kart entered checkpoint after race finished!")
            return

        if not self.race_started:
            # The race hasn't started yet, somebody cheated
            logger.warning("Kart entered checkpoint before race started!")
            return
        ...
        if entered_checkpoint is not kart.checkpoint:
            ...
            if entered_checkpoint is next_checkpoint:
                ...
                if lapped:
                    ...
                    pub.sendMessage("kart.lap-complete",
                                    kart_id=kart.id,
                                    lap=len(laps) + 1)
                    if len(laps) == self.number_of_laps:
                        logger.debug("%s{%s} has won the race!" %
                                     (kart.perf.name, kart.id))
...
class TrackScene(Scene):
    ...
    def setup(self, options):
        ...
        # find platforms and assign a texture to them
        for layer in self.tmx_data.objectgroups:
            if layer.name.lower() == 'terrain':
        ...
        shapes = load_shapes(self.tmx_data, space=self.space,
                             objects_xml_path=self.resources.objects_xml_path)
        ...
        if hasattr(self.tmx_data, "number_of_laps"):
            num_laps = int(self.tmx_data.number_of_laps)
        else:
            num_laps = 4

        self.race_manager = RaceManager(self, semaphore, num_laps, checkpoints)
        self.race_manager.subscribe_events()

        small_font = pygame.font.Font(self.resources.fonts['dimitri'], 64)
        large_font = pygame.font.Font(self.resources.fonts['dimitri'], 96)
        s = TextSprite("Time", font=small_font)
        self.hudgroup.add(s)

        s = RaceTimeSprite(self.race_manager, font=large_font)
        s.rect.move_ip(0, 100)
        self.hudgroup.add(s)
        ...
        for i, kart_id in enumerate(options["karts"]):
            ...
            if kart_id in player_karts:
                ...
                vp = RaceViewPort(self.resources, kart.id, self.race_manager)
                vp.follow(kart)
                self.vpgroup.add(vp)
